...
class otherPath{
  constructor(info){   ; 
    let message = this.differentiate(info.toString().split('\r\n\r\n'));
    console.log('message:',message);
    
  }
  differentiate(info){ 
      let headArray = info[0].split('\r\n')
      let state = headArray.shift();
      let stateLinInfp = this.stateLin(state);
      let headInfo = this.headInfo(headArray); 
      let bodyInfo = this.bodyInfo(info[1]);
      let meesage = Object.assign(stateLinInfp,headInfo,bodyInfo);
      return meesage
  ...
  stateLin(state){ 
    ...
    return {
      'statusCode':stateText[stateText.length - 2],
      'statusTxet':stateText[stateText.length - 1],
    }
  }
  headInfo(info){ 
    let headObj = info.map(item =>{
      let info = item.split(':'); 
      info = `"${info[0]}":"${info[1]}"`
      return info
    }) 
    headObj = JSON.parse(`{${headObj}}`) ;
